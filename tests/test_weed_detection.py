"""
KisanAI OS - AI Weed Detection milestone tests.
...
def test_weed_detect_requires_auth(client):
...
def test_model_not_configured_status(client, admin_headers):
    ...
    assert body["status"] == "MODEL_NOT_CONFIGURED"
    assert body["message"] == "Weed detection model is not configured"


def test_no_fake_prediction(client, admin_headers):
    """'No model' must never look like 'no weed' or 'weed detected'."""
    body = _detect(client, admin_headers, "plant.png", _png_bytes()).json()
    assert body["status"] not in ("WEED_DETECTED",)
    assert body["weed_name"] is None
...
def test_model_path_set_but_no_loader_still_not_configured(client, admin_headers, monkeypatch):
    ...
    fabricate results."""
    monkeypatch.setattr(settings, "WEED_MODEL_PATH", "models/whatever.pt")
    body = _detect(client, admin_headers, "plant.png", _png_bytes()).json()
    assert body["status"] == "MODEL_NOT_CONFIGURED"
    assert body["weed_name"] is None
    assert body["confidence"] is None
