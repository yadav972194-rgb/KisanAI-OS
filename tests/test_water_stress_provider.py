"""
KisanAI OS - real local ONNX crop-water-stress detection provider tests.
...
``WATER_STRESS_DETECTED`` result with correct preprocessing/label
...
fabricating a crop water stress level.
...
from config.core.providers.base import UnavailableWaterStressProvider
...
def _real_png_bytes():
    ...
    buf = io.BytesIO()
    Image.new("RGB", (48, 48), (90, 140, 60)).save(buf, format="PNG")
    return buf.getvalue()


def _write_model_and_labels(
    tmp_path,
    labels=("NO_STRESS", "MILD_STRESS", "MODERATE_STRESS", "SEVERE_STRESS"),
):
    model = tmp_path / "water.onnx"
    model.write_bytes(b"\x00fake-onnx-model-bytes")
    labels_file = tmp_path / "water.txt"
...
def test_unavailable_provider_returns_not_configured(monkeypatch):
    monkeypatch.setattr(settings, "WATER_STRESS_MODEL_PATH", "")
    monkeypatch.setattr(settings, "WATER_STRESS_MODEL_LABELS", "")
    provider = get_water_stress_provider()
    assert isinstance(provider, UnavailableWaterStressProvider)
...
def test_provider_factory_returns_unavailable_when_no_path(monkeypatch):
    monkeypatch.setattr(
        settings, "WATER_STRESS_MODEL_PATH", "models/does-not-exist.onnx"
    )
    assert isinstance(
        get_water_stress_provider(), UnavailableWaterStressProvider
    )
...
def test_provider_factory_existing_model_returns_real_provider(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        settings, "WATER_STRESS_MODEL_PATH", model
    )
    monkeypatch.setattr(
        settings, "WATER_STRESS_MODEL_LABELS", labels
    )
    ...
    provider = get_water_stress_provider()
    assert isinstance(provider, OnnxWaterStressProvider)
...
def test_onnx_provider_loads_model_successfully(monkeypatch, tmp_path):
    ...
    provider = OnnxWaterStressProvider(model, labels_path=labels)
    result = provider.detect(str(image), "Wheat")
    assert result["status"] == "WATER_STRESS_DETECTED"
    assert provider._layout == "NHWC"
    assert provider._session is not None


def test_onnx_provider_successful_detection_returns_stress_confidence(
    ...
    result = OnnxWaterStressProvider(model, labels_path=labels).detect(
        str(image), "Wheat"
    )

    assert result["status"] == "WATER_STRESS_DETECTED"
    assert result["stress_level"] == "MILD_STRESS"
    # softmax([0.05, 0.85, 0.05, 0.05]) -> argmax is class 1
    assert result["confidence"] == pytest.approx(0.4259, abs=0.001)
    assert result["model"] == "onnx:water.onnx"
    assert result["crop"] == "Wheat"
...
def test_onnx_provider_missing_model_file_returns_not_configured(monkeypatch, tmp_path):
    _install_fake_onnxruntime(monkeypatch)
    labels = tmp_path / "water.txt"
    labels.write_text("NO_STRESS\nMILD_STRESS\n", encoding="utf-8")
    image = tmp_path / "plant.png"
    image.write_bytes(_real_png_bytes())
    ...
    result = OnnxWaterStressProvider(
...
def test_onnx_provider_missing_labels_returns_not_configured(monkeypatch, tmp_path):
    ...
    result = OnnxWaterStressProvider(str(model)).detect(str(image))
    assert result["status"] == "MODEL_NOT_CONFIGURED"
    assert result["stress_level"] is None
    assert result["confidence"] is None
...
def test_onnx_provider_label_count_mismatch_returns_not_configured(monkeypatch, tmp_path):
    ...
    model, _ = _write_model_and_labels(
        tmp_path, labels=("NO_STRESS", "MILD_STRESS")
    )
    image = tmp_path / "plant.png"
    image.write_bytes(_real_png_bytes())
    ...
    result = OnnxWaterStressProvider(model).detect(str(image))
    assert result["status"] == "MODEL_NOT_CONFIGURED"
...
def test_onnx_provider_corrupt_model_returns_not_configured(monkeypatch, tmp_path):
    ...
    provider = OnnxWaterStressProvider(str(model), labels_path=str(labels))
...
def test_onnx_provider_invalid_image_path_returns_error(monkeypatch, tmp_path):
    """A missing image must not fabricate a water stress level."""
    _install_fake_onnxruntime(monkeypatch)
    model, labels = _write_model_and_labels(tmp_path)
