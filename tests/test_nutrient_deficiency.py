"""
KisanAI OS - AI Nutrient Deficiency Detection milestone tests.
...
def test_no_fake_prediction(client, admin_headers):
    """'No model' must never look like 'no deficiency' or 'deficiency
    detected'."""
    body = _detect(client, admin_headers, "plant.png", _png_bytes()).json()
    assert body["status"] not in ("NUTRIENT_DEFICIENCY_DETECTED",)
    assert body["deficiency_name"] is None
...
def test_model_path_set_but_no_loader_still_not_configured(client, admin_headers, monkeypatch):
    ...
    fabricate results."""
    monkeypatch.setattr(
        settings, "NUTRIENT_DEFICIENCY_MODEL_PATH", "models/whatever.pt"
    )
    body = _detect(client, admin_headers, "plant.png", _png_bytes()).json()
    assert body["status"] == "MODEL_NOT_CONFIGURED"
    assert body["deficiency_name"] is None
