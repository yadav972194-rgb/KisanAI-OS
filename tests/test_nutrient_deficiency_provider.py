"""
KisanAI OS - real local ONNX nutrient-deficiency detection provider tests.
...
``NUTRIENT_DEFICIENCY_DETECTED`` result with correct
preprocessing/label mapping, and that every missing/broken
configuration degrades safely to the honest ``MODEL_NOT_CONFIGURED``
result instead of crashing or fabricating a nutrient-deficiency
identification.
...
from config.core.providers.base import UnavailableNutrientDeficiencyProvider
...
def _write_model_and_labels(tmp_path, labels=("Nitrogen", "Phosphorus", "Potassium")):
    model = tmp_path / "nutrient.onnx"
    model.write_bytes(b"\x00fake-onnx-model-bytes")
    labels_file = tmp_path / "nutrient.txt"
...
def test_unavailable_provider_returns_not_configured(monkeypatch):
    monkeypatch.setattr(settings, "NUTRIENT_DEFICIENCY_MODEL_PATH", "")
    monkeypatch.setattr(settings, "NUTRIENT_DEFICIENCY_MODEL_LABELS", "")
    provider = get_nutrient_deficiency_provider()
    assert isinstance(provider, UnavailableNutrientDeficiencyProvider)

    result = provider.detect("/nonexistent.png", "Wheat")
    assert result["status"] == "MODEL_NOT_CONFIGURED"
    assert result["deficiency_name"] is None
...
def test_provider_factory_returns_unavailable_when_no_path(monkeypatch):
    monkeypatch.setattr(
        settings,
        "NUTRIENT_DEFICIENCY_MODEL_PATH",
        "models/does-not-exist.onnx",
    )
    assert isinstance(
        get_nutrient_deficiency_provider(),
...
def test_provider_factory_existing_model_returns_real_provider(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        settings, "NUTRIENT_DEFICIENCY_MODEL_PATH", model
    )
    monkeypatch.setattr(
        settings, "NUTRIENT_DEFICIENCY_MODEL_LABELS", labels
    )
    ...
    provider = get_nutrient_deficiency_provider()
    assert isinstance(provider, OnnxNutrientDeficiencyProvider)
...
def test_onnx_provider_loads_model_successfully(monkeypatch, tmp_path):
    ...
    provider = OnnxNutrientDeficiencyProvider(model, labels_path=labels)
    result = provider.detect(str(image), "Wheat")
    assert result["status"] == "NUTRIENT_DEFICIENCY_DETECTED"
    assert provider._layout == "NHWC"
    assert provider._session is not None


def test_onnx_provider_successful_detection_returns_name_confidence(
    ...
    result = OnnxNutrientDeficiencyProvider(model, labels_path=labels).detect(
        str(image), "Wheat"
    )

    assert result["status"] == "NUTRIENT_DEFICIENCY_DETECTED"
    assert result["deficiency_name"] == "Phosphorus"
    # softmax([0.1, 0.85, 0.05]) -> argmax is class 1 at ~0.5204
    assert result["confidence"] == pytest.approx(0.5204, abs=0.001)
    assert result["model"] == "onnx:nutrient.onnx"
    assert result["crop"] == "Wheat"
...
def test_onnx_provider_missing_model_file_returns_not_configured(monkeypatch, tmp_path):
    _install_fake_onnxruntime(monkeypatch)
    labels = tmp_path / "nutrient.txt"
    labels.write_text("Nitrogen\nPhosphorus\n", encoding="utf-8")
    image = tmp_path / "plant.png"
    image.write_bytes(_real_png_bytes())
    ...
    result = OnnxNutrientDeficiencyProvider(
...
def test_onnx_provider_missing_labels_returns_not_configured(monkeypatch, tmp_path):
    ...
    result = OnnxNutrientDeficiencyProvider(str(model)).detect(str(image))
    assert result["status"] == "MODEL_NOT_CONFIGURED"
    assert result["deficiency_name"] is None
    assert result["confidence"] is None
...
def test_onnx_provider_label_count_mismatch_returns_not_configured(monkeypatch, tmp_path):
    ...
    model, _ = _write_model_and_labels(
        tmp_path, labels=("Nitrogen", "Phosphorus")
    )
    image = tmp_path / "plant.png"
    image.write_bytes(_real_png_bytes())
    ...
    result = OnnxNutrientDeficiencyProvider(model).detect(str(image))
    assert result["status"] == "MODEL_NOT_CONFIGURED"
...
def test_onnx_provider_corrupt_model_returns_not_configured(monkeypatch, tmp_path):
    ...
    provider = OnnxNutrientDeficiencyProvider(str(model), labels_path=str(labels))
...
def test_onnx_provider_invalid_image_path_returns_error(monkeypatch, tmp_path):
    """A missing image must not fabricate a deficiency identification."""
    _install_fake_onnxruntime(monkeypatch)
    model, labels = _write_model_and_labels(tmp_path)
