"""
KisanAI OS - AI Crop Water Stress Detection milestone tests.
...
def test_no_fake_prediction(client, admin_headers):
    """'No model' must never look like 'no stress' or 'stress detected'."""
    body = _detect(client, admin_headers, "plant.png", _png_bytes()).json()
    assert body["status"] not in ("WATER_STRESS_DETECTED",)
...
def test_model_path_set_but_no_loader_still_not_configured(client, admin_headers, monkeypatch):
    ...
    fabricate results."""
    monkeypatch.setattr(
        settings, "WATER_STRESS_MODEL_PATH", "models/whatever.pt"
    )
    body = _detect(client, admin_headers, "plant.png", _png_bytes()).json()
    assert body["status"] == "MODEL_NOT_CONFIGURED"
    assert body["stress_level"] is None
