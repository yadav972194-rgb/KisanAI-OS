...
import 'package:kisanai/models/water_stress.dart';
...
void main() {
  (DetectionController<WaterStressResult>, FakeBackend) build({
    ...
    final controller = WaterStressController(WaterStressApi(client), picker);
    return (controller, backend);
  }

  group('WaterStressController', () {
    ...
    test('detected water stress parses correctly', () async {
      final (controller, _) = build(
        picker: () async => _leaf,
        path: '/api/water-stress/detect',
      ...
      expect(controller.result!.stressLevel, 'sample-name');
      expect(controller.result!.confidence, 0.87);
    });

    test('network failure surfaces connection error', () async {
      final controller = WaterStressController(
        _ThrowingWaterApi(ApiClient(
          baseUrl: 'http://test.local',
...
class _ThrowingWaterApi extends WaterStressApi {
  _ThrowingWaterApi(super.client);
