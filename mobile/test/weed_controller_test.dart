...
import 'package:kisanai/models/weed_detection.dart';
...
void main() {
  (DetectionController<WeedDetectionResult>, FakeBackend) build({
    ...
    final controller = WeedController(WeedDetectionApi(client), picker);
    return (controller, backend);
  }

  group('WeedController', () {
    ...
    test('detected weed parses correctly', () async {
      final (controller, _) = build(
        picker: () async => _leaf,
        path: '/api/weed/detect',
      ...
      expect(controller.result!.weedName, 'sample-name');
      expect(controller.result!.confidence, 0.87);
    });

    test('network failure surfaces connection error', () async {
      final controller = WeedController(
        _ThrowingWeedApi(ApiClient(
          baseUrl: 'http://test.local',
...
class _ThrowingWeedApi extends WeedDetectionApi {
  _ThrowingWeedApi(super.client);
