"""
KisanAI OS
Crop Water Stress Detection Service
Version: 1.0.0

Coordinates the AI crop-water-stress detection flow without depending
on any ML framework:

    Image -> UploadService validation/storage (existing layer)
          -> WaterStressService
          -> WaterStressProvider (replaceable)
...
controlled ``MODEL_NOT_CONFIGURED`` result - never a fake stress level.
"""

import os

from config.core.logger import logger
from config.core.providers import get_water_stress_provider
...
class WaterStressError(Exception):
    """Crop-water-stress detection could not be completed (provider
    failure)."""
...
class WaterStressService:
    ...
    def __init__(self, upload_service=None, provider=None):
        ...
        self.provider = provider or get_water_stress_provider()
    ...
    def detect(self, file, crop_name=None):
        ...
        image) or ``WaterStressError`` (provider failure).
        """
        ...
        logger.info(
            "Crop water stress detection request: stored image %s "
            "(crop=%s)",
            stored_name,
            crop_name,
        )
        ...
        except WaterStressError:
            raise
        except Exception as error:  # never leak provider internals
            logger.exception("Crop water stress provider failed")
            raise WaterStressError(
                "Crop water stress detection service unavailable"
            ) from error
    ...
    def _normalize(self, result, crop_name):
        ...
        return {
            ...
            "status": status,
            "crop": crop,
            "stress_level": result.get(RESULT_STRESS_LEVEL),
