"""
KisanAI OS
ONNX Nutrient Deficiency Detection Provider

Runs a real, local, free crop nutrient-deficiency image classifier from
an ONNX model file. No external API and no paid service.

Mirrors the proven disease-detection, pest-detection and weed-detection
providers exactly (same loading, preprocessing and honesty contract).

Runtime requirements (all free / open source):
  - ``onnxruntime``: CPU inference runtime.
  - ``Pillow``:      image decode + preprocessing.
  - ``NUTRIENT_DEFICIENCY_MODEL_PATH``: a ``.onnx`` model exporting one
    image input tensor and one score/logit output tensor.
  - a labels file (one class name per line, in the model's output
    order) - either ``<model>.txt`` next to the model or the
    ``NUTRIENT_DEFICIENCY_MODEL_LABELS`` setting.

The provider never fabricates a nutrient-deficiency identification. If
the model file is missing/corrupt, the runtime is unavailable, the
labels file is missing or the output shape does not match the labels,
it returns the controlled ``MODEL_NOT_CONFIGURED`` result so the API
and the app stay honest (never confused with "no deficiency" or a real
...
from config.core.providers.base import (
    ...
    STATUS_NUTRIENT_DEFICIENCY_DETECTED,
    NutrientDeficiencyProvider,
)
...
class OnnxNutrientDeficiencyProvider(NutrientDeficiencyProvider):
    """Real local crop nutrient-deficiency classifier backed by an ONNX
    model."""
    ...
    def _ensure_loaded(self):
        ...
        try:
            ...
            logger.warning(
                "onnxruntime unavailable; nutrient deficiency detection "
                "not configured (%s)",
                error,
            )
            self._load_error = (
        ...
        try:
            ...
            if not self._labels:
                return self._fail(
                    "Nutrient deficiency labels file is missing or empty: "
                    "%s" % self.labels_path
                )

            if self._num_classes and len(self._labels) != self._num_classes:
                return self._fail(
                    "Nutrient deficiency labels count (%d) does not match "
                    "the model output (%d)"
                    % (len(self._labels), self._num_classes)
                )

            logger.info(
                "Nutrient deficiency model loaded: %s (%s, %d classes)",
                self.model_path,
                self._layout,
                len(self._labels),
            )
            ...
            logger.warning(
                "Failed to load nutrient deficiency model %s: %s",
                self.model_path,
                error,
            )
            self._load_error = (
    ...
    def detect(self, image_path: str, crop_name: str | None = None) -> dict:
        ...
        logger.info(
            "Nutrient deficiency detection: %s (crop=%s) -> %s @ %.2f",
        ...
        return {
            RESULT_STATUS: STATUS_NUTRIENT_DEFICIENCY_DETECTED,
            RESULT_CROP: crop_name,
            RESULT_DEFICIENCY_NAME: label,
