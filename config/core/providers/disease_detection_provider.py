"""
KisanAI OS
ONNX Disease Detection Provider

Runs a real, local, free crop-disease image classifier from an ONNX
...
Runtime requirements (all free / open source):
  ...
  - ``DISEASE_MODEL_PATH``: a ``.onnx`` model exporting one image input
  ...
  - a labels file (one class name per line, in the model's output
    ...
    ``DISEASE_MODEL_LABELS`` setting.

The provider never fabricates a diagnosis. If the model file is
...
honest (never confused with "healthy" or a real diagnosis).
"""
...
class OnnxDiseaseDetectionProvider(DiseaseDetectionProvider):
    """Real local crop-disease classifier backed by an ONNX model."""
    ...
    def _ensure_loaded(self):
        ...
        try:
            ...
            logger.warning(
                "onnxruntime unavailable; disease detection not "
                "configured (%s)",
                error,
            )
            self._load_error = "Disease detection model is not configured"
            return self._load_error
        ...
        try:
            ...
            if not self._labels:
                return self._fail(
                    "Disease detection labels file is missing or empty: "
                    "%s" % self.labels_path
                )

            if self._num_classes and len(self._labels) != self._num_classes:
                return self._fail(
                    "Disease detection labels count (%d) does not match "
                    "the model output (%d)"
                    % (len(self._labels), self._num_classes)
                )

            logger.info(
                "Disease detection model loaded: %s (%s, %d classes)",
                self.model_path,
                self._layout,
                len(self._labels),
            )
            ...
            logger.warning(
                "Failed to load disease detection model %s: %s",
                self.model_path,
                error,
            )
            self._load_error = "Disease detection model is not configured"
    ...
    def detect(self, image_path: str, crop_name: str | None = None) -> dict:
        ...
        logger.info(
            "Disease detection: %s (crop=%s) -> %s @ %.2f",
