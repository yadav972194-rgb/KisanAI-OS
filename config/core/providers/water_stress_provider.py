"""
KisanAI OS
ONNX Crop Water Stress Detection Provider

Runs a real, local, free crop water stress image classifier from an
ONNX model file. No external API and no paid service.

Mirrors the proven disease-detection, pest-detection, weed-detection,
nutrient-deficiency and growth-stage providers exactly (same loading,
...
Runtime requirements (all free / open source):
  ...
  - ``WATER_STRESS_MODEL_PATH``: a ``.onnx`` model exporting one image
  ...
  - a labels file (one class name per line, in the model's output
    ...
    ``WATER_STRESS_MODEL_LABELS`` setting. Labels normally match the
    stable stress scale: NO_STRESS / MILD_STRESS / MODERATE_STRESS /
    SEVERE_STRESS.

The provider never fabricates a water stress level. If the model file is
...
honest (never confused with "no stress" or a real stress
identification).
"""
...
class OnnxWaterStressProvider(WaterStressProvider):
    """Real local crop water stress classifier backed by an ONNX model."""
    ...
    def _ensure_loaded(self):
        ...
        try:
            ...
            logger.warning(
                "onnxruntime unavailable; crop water stress detection "
                "not configured (%s)",
                error,
            )
            self._load_error = (
                "Crop water stress model is not configured"
            )
            return self._load_error
        ...
        try:
            ...
            if not self._labels:
                return self._fail(
                    "Crop water stress labels file is missing or empty: "
                    "%s" % self.labels_path
                )

            if self._num_classes and len(self._labels) != self._num_classes:
                return self._fail(
                    "Crop water stress labels count (%d) does not match "
                    "the model output (%d)"
                    % (len(self._labels), self._num_classes)
                )

            logger.info(
                "Crop water stress model loaded: %s (%s, %d classes)",
                self.model_path,
                self._layout,
                len(self._labels),
            )
            ...
            logger.warning(
                "Failed to load crop water stress model %s: %s",
                self.model_path,
                error,
            )
            self._load_error = (
    ...
    def detect(self, image_path: str, crop_name: str | None = None) -> dict:
        ...
        logger.info(
            "Crop water stress detection: %s (crop=%s) -> %s @ %.2f",
        ...
        return {
            RESULT_STATUS: STATUS_WATER_STRESS_DETECTED,
            RESULT_CROP: crop_name,
            RESULT_STRESS_LEVEL: label,
