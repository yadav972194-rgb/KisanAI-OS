"""
KisanAI OS
ONNX Pest Detection Provider

Runs a real, local, free crop-pest image classifier from an ONNX
model file. No external API and no paid service.

Mirrors the proven disease-detection provider exactly (same loading,
preprocessing and honesty contract).

Runtime requirements (all free / open source):
  - ``onnxruntime``: CPU inference runtime.
  - ``Pillow``:      image decode + preprocessing.
  - ``PEST_MODEL_PATH``: a ``.onnx`` model exporting one image input
  ...
  - a labels file (one class name per line, in the model's output
    ...
    ``PEST_MODEL_LABELS`` setting.

The provider never fabricates a pest identification. If the model file is
...
honest (never confused with "no pest" or a real pest identification).
"""
...
class OnnxPestDetectionProvider(PestDetectionProvider):
    """Real local crop-pest classifier backed by an ONNX model."""
    ...
    def _ensure_loaded(self):
        ...
        try:
            ...
            logger.warning(
                "onnxruntime unavailable; pest detection not "
                "configured (%s)",
                error,
            )
            self._load_error = "Pest detection model is not configured"
            return self._load_error
        ...
        try:
            ...
            if not self._labels:
                return self._fail(
                    "Pest detection labels file is missing or empty: "
                    "%s" % self.labels_path
                )

            if self._num_classes and len(self._labels) != self._num_classes:
                return self._fail(
                    "Pest detection labels count (%d) does not match "
                    "the model output (%d)"
                    % (len(self._labels), self._num_classes)
                )

            logger.info(
                "Pest detection model loaded: %s (%s, %d classes)",
                self.model_path,
                self._layout,
                len(self._labels),
            )
            ...
            logger.warning(
                "Failed to load pest detection model %s: %s",
                self.model_path,
                error,
            )
            self._load_error = "Pest detection model is not configured"
    ...
    def detect(self, image_path: str, crop_name: str | None = None) -> dict:
        ...
        return {
            RESULT_STATUS: STATUS_PEST_DETECTED,
            RESULT_CROP: crop_name,
            RESULT_PEST_NAME: label,
