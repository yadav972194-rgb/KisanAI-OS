"""
KisanAI OS
ONNX Weed Detection Provider

Runs a real, local, free crop-weed image classifier from an ONNX
model file. No external API and no paid service.

Mirrors the proven disease-detection and pest-detection providers
exactly (same loading, preprocessing and honesty contract).

Runtime requirements (all free / open source):
  - ``onnxruntime``: CPU inference runtime.
  - ``Pillow``:      image decode + preprocessing.
  - ``WEED_MODEL_PATH``: a ``.onnx`` model exporting one image input
    tensor and one score/logit output tensor.
  - a labels file (one class name per line, in the model's output
    order) - either ``<model>.txt`` next to the model or the
    ``WEED_MODEL_LABELS`` setting.

The provider never fabricates a weed identification. If the model file
is missing/corrupt, the runtime is unavailable, the labels file is
missing or the output shape does not match the labels, it returns the
controlled ``MODEL_NOT_CONFIGURED`` result so the API and the app stay
honest (never confused with "no weed" or a real weed identification).
"""
...
from config.core.providers.base import (
    ...
    STATUS_WEED_DETECTED,
    WeedDetectionProvider,
)
...
class OnnxWeedDetectionProvider(WeedDetectionProvider):
    """Real local crop-weed classifier backed by an ONNX model."""
    ...
    def _ensure_loaded(self):
        ...
        try:
            ...
            logger.warning(
                "onnxruntime unavailable; weed detection not "
                "configured (%s)",
                error,
            )
            self._load_error = "Weed detection model is not configured"
            return self._load_error
        ...
        try:
            ...
            if not self._labels:
                return self._fail(
                    "Weed detection labels file is missing or empty: "
                    "%s" % self.labels_path
                )

            if self._num_classes and len(self._labels) != self._num_classes:
                return self._fail(
                    "Weed detection labels count (%d) does not match "
                    "the model output (%d)"
                    % (len(self._labels), self._num_classes)
                )

            logger.info(
                "Weed detection model loaded: %s (%s, %d classes)",
                self.model_path,
                self._layout,
                len(self._labels),
            )
            ...
            logger.warning(
                "Failed to load weed detection model %s: %s",
                self.model_path,
                error,
            )
            self._load_error = "Weed detection model is not configured"
    ...
    def detect(self, image_path: str, crop_name: str | None = None) -> dict:
        ...
        return {
            RESULT_STATUS: STATUS_WEED_DETECTED,
            RESULT_CROP: crop_name,
            RESULT_WEED_NAME: label,
